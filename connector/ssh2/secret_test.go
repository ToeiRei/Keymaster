...
func TestParseSecret_RoundTrip(t *testing.T) {
	...
	fromValues, err := c.ParseSecretFromFields(map[string]string{
...
func TestParseSecretFromFields_Validates(t *testing.T) {
	c := &Connector{}
	...
	for name, values := range map[string]map[string]string{
		...
		if _, err := c.ParseSecretFromFields(values); err == nil {
			t.Fatalf("expected ParseSecretFromFields to reject %s", name)
		}
	}
}
...
func TestParseSecretFromFields_DerivesPublicKeyFromEncryptedKey(t *testing.T) {
	c := &Connector{}
	encryptedKey, err := generateEncryptedTestPrivateKeyPEM("hunter2")
	if err != nil {
		t.Fatalf("generate encrypted test private key: %v", err)
	...
	parsed, err := c.ParseSecretFromFields(map[string]string{
...
func TestParseSecretFromFields_OmitsPassphraseButKeepsPublicKey(t *testing.T) {
	c := &Connector{}
	encryptedKey, err := generateEncryptedTestPrivateKeyPEM("hunter2")
	if err != nil {
		t.Fatalf("generate encrypted test private key: %v", err)
...
func TestNarrowSecret_RejectsForeignSecret(t *testing.T) {
	if _, err := narrowSecret(foreignSecret{}); err == nil {
		t.Fatal("expected narrowSecret to reject a secret from another connector")
	}
}
