...
func initConfig() error {
	if cfgFile != "" {
		...
		// 1. Prefer platform-specific config directory (e.g., ~/.config/keymaster)
		configDir, err := os.UserConfigDir()
		if err == nil {
			keymasterConfigDir := filepath.Join(configDir, "keymaster")
			viper.AddConfigPath(keymasterConfigDir)
			viper.SetConfigName("config") // e.g., config.yaml
			viper.SetConfigType("yaml")
		}

		// 2. Fallback to current directory for backward compatibility
		viper.AddConfigPath(".")
		viper.SetConfigType("yaml")
		viper.SetConfigName(".keymaster") // old name
	}
